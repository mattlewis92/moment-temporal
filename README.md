...
- `moment.tz.names()` works out of the box: the name/link registries are
  seeded at import from a small generated table (`src/tz/seed-data.js`,
  regenerated by `scripts/gen-seed-data.mjs` — names only, no offset
  timelines), while all zone *math* still comes from the host database.
- Country data stays load-only: `moment.tz.countries()` returns `[]` and
  `zonesForCountry()`/`zone.countries()` need `tz.load()` — zone→country
  mappings aren't exposed by any web API.
